    ...
    <main className="site-shell overflow-hidden">
      <header className="site-header px-4 py-4 sm:px-8 sm:py-5 lg:px-12">
        <nav className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 border-b border-ink/15 pb-4">
          <a className="brand-mark max-w-[15rem] text-sm font-semibold leading-tight sm:max-w-none sm:text-base" href="#top">Bille L.</a>
          <div className="flex items-center gap-1 overflow-x-auto text-sm text-ink/65 sm:overflow-visible">
            <a className="nav-link rounded-md px-3 py-2 hover:bg-ink/5" href="#stack">Stack</a>
            <a className="nav-link rounded-md px-3 py-2 hover:bg-ink/5" href="#projects">Projects</a>
            <a className="nav-link rounded-md px-3 py-2 hover:bg-ink/5" href="#story">Story</a>
          </div>
        </nav>
      </header>

      <section id="top" className="hero-section px-4 pb-14 pt-8 sm:px-8 sm:pb-20 sm:pt-10 lg:px-12 lg:pt-16">
        <div className="hero-signal" aria-hidden="true" />
        <div className="mx-auto grid max-w-7xl gap-9 lg:grid-cols-[0.72fr_1.28fr] lg:gap-12">
          <aside className="hero-rail motion-enter border-l-4 border-accent pl-5 sm:pl-6" style={motionDelay(0, 80)}>
            ...
            <div className="mt-7 space-y-4 text-sm text-ink/70 sm:mt-8">
              <p className="detail-line flex items-center gap-2"><MapPin className="size-4 shrink-0" /> Available for product work</p>
              <p className="detail-line flex items-center gap-2"><Code2 className="size-4 shrink-0" /> React, Node.js, data workflows</p>
              <p className="detail-line flex items-center gap-2"><PenTool className="size-4 shrink-0" /> Product-minded UI delivery</p>
            </div>
          </aside>

          <div className="min-w-0">
            <h1 className="motion-enter font-serif text-[clamp(2.9rem,15vw,5rem)] leading-[0.95] text-ink sm:text-3xl lg:text-4xl" style={motionDelay(1, 80)}>
              Useful software for teams, tools, and products that need momentum
            </h1>
            <p className="motion-enter mt-6 max-w-3xl text-lg leading-8 text-ink/70 sm:mt-7 sm:text-xl sm:leading-9" style={motionDelay(2, 80)}>
            ...
            <div className="motion-enter mt-9 flex flex-col gap-3 sm:flex-row" style={motionDelay(3, 80)}>
        ...
        <div className="mx-auto grid max-w-7xl gap-9 lg:grid-cols-[0.7fr_1.3fr] lg:gap-10">
          ...
          <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 sm:gap-3 xl:grid-cols-4">
            {stack.map((item, index) => (
              <div key={item} className="stack-chip flex min-h-14 items-center justify-center rounded-md border border-ink/15 bg-ink/[0.04] px-3 text-center text-sm font-semibold sm:min-h-16" style={motionDelay(index, 0, 35)}>{item}</div>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            {projects.map((project, index) => (
              <Card key={project.title} className="project-card scroll-reveal border-ink/10 bg-card/85" style={motionDelay(index, 80)}>
        ...
        <div className="mx-auto grid max-w-7xl gap-9 border-t border-ink/15 pt-10 sm:pt-12 lg:grid-cols-[0.55fr_1.45fr] lg:gap-10">
          ...
          <div className="grid gap-4">
            {timeline.map((item, index) => (
              <article key={item.title} className="timeline-item scroll-reveal grid gap-3 border border-ink/10 bg-card/75 p-4 sm:grid-cols-[9rem_1fr] sm:gap-4 sm:p-5" style={motionDelay(index, 60)}>
